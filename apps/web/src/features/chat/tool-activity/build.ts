...
import { activityMetadata, activityArgs, activityTool } from "./metadata";
...
export function buildPipeline(events: ActivityEventDTO[], live = false): { steps: PipelineStep[]; tx: PipelineTx[] } {
  ...
  for (const ev of ordered) {
    ...
    const metadata = activityMetadata(p);
    if (ev.type === "tool.preparing" || ev.type === "tool.started") {
      const tool = activityTool(p);
      // Deep Research (web:) dirender sebagai kartu sumber tersendiri, bukan
      // step pipeline router — lihat buildRunTimeline + ResearchCard.
      if (tool === "web:search") continue;
      const args = activityArgs(p);
      const callKey = String(p.callId ?? ev.activityId);
      const existing = byCall.get(callKey);
      if (existing) {
        Object.assign(existing, metadata);
        if (args) existing.args = args;
        existing.label = humanizeTool(tool, existing.args, existing);
        continue;
      }
      const step: PipelineStep = {
        key: `${ev.id}`,
        callId: p.callId ? String(p.callId) : undefined,
        index: steps.length + 1,
        label: humanizeTool(tool, args, metadata),
        args,
        ...metadata,
        tool,
        status: "running",
      ...
      if (String(p.tool ?? p.name ?? "") === "web:search") continue;
      const callKey = String(p.callId ?? ev.activityId);
      ...
      if (step) {
        ...
        if (typeof p.durationMs === "number") step.durationMs = p.durationMs + (typeof p.preparationMs === "number" ? p.preparationMs : 0);
        if (typeof p.args === "string" && p.args) step.args = p.args;
        // Perkaya label dengan argumen nyata ("Membaca file package.json").
        step.label = humanizeTool(step.tool, step.args, step);
      } else {
